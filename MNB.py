...
def likelihoods (tokened_text, labels,smoothing_val=1, test_text=[]): #tokens probabilities
    ...
    for c in classes_names:
        ...
        for w in voc_words:
            likelihood[c][w] = (flat_list.count(w)+smoothing_val)/(flat_list_length+voc_size) #claculating probability for each words in each class
    return (likelihood)
...
if __name__ == '__main__':
    ...
    #present results
    MNB_pred = MNB_predict (X[100:],y[100:],X[:100], raw=True)
    true = y[:100]
